...
class_iou = dict()
class_iou_2class = dict()
class_iou_2class['bg'] = list()
class_iou_2class['fg'] = list()
...
for i, (name1, name2) in enumerate(zip(test_label_names, prediction_names)):
    ...
    label_2class = np.clip(label, 0, 1)
    pred_2class = np.clip(pred, 0, 1)
    confusion_matrix = multilabel_confusion_matrix(label_2class.flatten(), pred_2class.flatten(), labels=[0, 1])
    
    (tn, fp), (fn, tp) = confusion_matrix[0]
    if tp + fn > 0:
        class_iou_2class['bg'].append(tp / (tp + fp + fn))
    (tn, fp), (fn, tp) = confusion_matrix[1]
    if tp + fn > 0:
        class_iou_2class['fg'].append(tp / (tp + fp + fn))
...
for eval_cls in ['bg', 'fg']:
    class_iou_2class[eval_cls] = np.mean(class_iou_2class[eval_cls])
    print('{cls:}: {iou:.4f}'.format(cls=eval_cls, iou=class_iou_2class[eval_cls]))

print('\n**********************************************')
print('* The Mean IoU of all classes is as follows: *')
print('**********************************************')
print('Mean IoU: {mean_iou:.4f}'.format(mean_iou=np.mean(list(class_iou.values()))))
print('Mean 2class 2class IoU: {mean_2class_iou:.4f}'.format(mean_2class_iou=np.mean(list(class_iou_2class.values()))))
