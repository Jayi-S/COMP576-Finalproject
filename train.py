...
parser.add_argument('--loss', help='The loss function for traing.', type=str, default=None,
                    choices=['ce', 'focal_loss', 'dice_ce_loss', 'weight_miou_loss','miou_loss', 'self_balanced_focal_loss', 'dice_loss'])
...
losses = {'ce': categorical_crossentropy_with_logits,
          ...
          'dice_loss': dice_loss(),
          'weight_miou_loss': weight_miou_loss(num_classes=args.num_classes),
...
model_checkpoint = tf.keras.callbacks.ModelCheckpoint(
    filepath=os.path.join(paths['checkpoints_path'],
                          '{model}_based_on_{base}_{loss}'.format(model=args.model, base=base_model, loss=args.loss) +
...
net.save(filepath=os.path.join(
    paths['weights_path'], '{model}_based_on_{base_model}_{loss}.h5'.format(model=args.model, base_model=base_model, loss=args.loss)))
